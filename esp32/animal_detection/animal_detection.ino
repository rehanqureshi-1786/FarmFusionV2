...
const char *SERVER_IP = "10.44.57.222";
...
const unsigned long SAMPLE_INTERVAL = 30; // Sample every 30ms (150ms window)
const unsigned long HOLD_TIME = 150;      // 150ms state hold before dispatch
...
struct DebouncedSensor {
  ...
  int sampleIndex;
  bool isDetected; // True = detected, False = detecting (clear)
...
void setup() {
  ...
  // Initialize sample buffers with initial pin reads
...
void loop() {
  ...
  if (currentMillis - lastSampleTime >= SAMPLE_INTERVAL) {
    ...
    for (int i = 0; i < NUM_SENSORS; i++) {
      ...
      // Hysteresis check: Require ALL 5 consecutive samples to match
      bool allMatch = true;
...
// ============================================================================
// HTTP POST REAL-TIME EVENT TELEMETRY (<10ms latency)
// ============================================================================
void sendDetectionEvent(const char *sensorName, const char *sensorType,
                        bool isDetected) {
...
// ============================================================================
// HTTP POST UNIFIED HEARTBEAT (Refreshes Node & All 8 Sensors every 1.5s)
// ============================================================================
void sendUnifiedHeartbeat() {
  WiFiClient wifiClient;
  HTTPClient http;
